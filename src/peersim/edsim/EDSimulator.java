...
public class EDSimulator{
	private static Logger LOGGER = Logger.getLogger( EDSimulator.class);
...
private static void scheduleControls()
{
	...
	LOGGER.warn("EDSimulator: loaded controls "+
		Arrays.asList(names));
...
private static boolean executeNext() {
	...
	if( ev == null )
	{
		LOGGER.warn("EDSimulator: queue is empty, quitting"+
	...
	if (time >= endtime)
	{
		LOGGER.warn("EDSimulator: reached end time, quitting,"+
...
public static void nextExperiment() 
{
	...
	// initialization
	LOGGER.warn("EDSimulator: resetting");
	...
	nextlog = 0;
	Network.reset();
	LOGGER.warn("EDSimulator: running initializers");
	runInitializers();
	scheduleControls();
