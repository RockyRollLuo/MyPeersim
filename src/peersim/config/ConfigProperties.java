...
public	ConfigProperties( String[] pars, String resource ) {
	...
	try
	{
		if( resource != null )
		{
			...
			LOGGER.warn("ConfigProperties: System resource "
			+resource+" loaded.");
		}
	}
	catch( Exception e )
	...
	for (int i=0; i < pars.length; i++)
	{
		...
		{
			load( pars[i] );
			LOGGER.warn("ConfigProperties: File "+pars[i]+" loaded.");
			pars[i] = "";
		}
		catch( IOException e )
		{
			try
			{
				...
				LOGGER.warn("ConfigProperties: Property '" +
					pars[i] + "' set.");
			}
			catch( Exception e2 )
			{
				LOGGER.error("ConfigProperties: " + e2 );
			}
		}
		catch( Exception e )
		{
